...
import logdata from './Indusrtydata';
import AccountCircleIcon from '@material-ui/icons/AccountCircle';
import axios from 'axios';
import DeleteIcon from '@material-ui/icons/Delete';
import EditIcon from '@material-ui/icons/Edit';
import Updatefarm from './mystockupdate';
...
const updatemyretForm=()=>
{
  ...
  const mydelete=id=>
  {
   ...
   var PId=0;
    ...
    else if(!/^[a-zA-Z]+-[a-zA-Z0-9-]*$/.test(values.PID))
    {
        document.getElementById("pid").hidden=false;
        document.getElementById("pid").innerHTML="Invalid format of id";
  ...
  if(/[0-9]/.test(idd))
    {
        ...
        document.getElementById("apname").innerHTML="Number are not allowed";
...
const uAddcheckname=e=>
{
    var idd=e.target.value;

    if(!/[A-Za-z]{3}[A-Za-z0-9]/.test(idd))
    {
        document.getElementById("upname").hidden=false;
        document.getElementById("upname").innerHTML="Invalid Name";
    }
    else if(idd=="")
    {
...
const uAddcheckquantity=e=>
{
  ...
  if(/[""]/.test(idd))
  {
      ...
      document.getElementById("upql").innerHTML="e";
   }
    else if(!/[0-9]/.test(idd))
    {
        document.getElementById("upql").hidden=false;
...
const uAddcheckDate=e=>
{
    var idd=e.target.value;
    var d=Date();
    count--;

    if(!/[0-9]+-[0-9]+-[0-9]/.test(idd))
    {
    ...
    {
        document.getElementById("upmdt").hidden=false;
        document.getElementById("upmdt").innerHTML="Please enter valid date";
...
/*const updateProduct = {
  ...
  const Addcheckquality=e=>
  {
    ...
    {
        document.getElementById("upqt").hidden=false;
        document.getElementById("upqt").innerHTML="Format should be XXX%";
  ...
  const AddcheckMDate=e=>
  {
     ...
     var uyy=idd[4]
      ...
      {
          document.getElementById("upmdt").hidden=false;
          document.getElementById("upmdt").innerHTML="Invalid date format(format:dd-mm-yyyy)";
      }
    ...
    var num=sessionStorage.getItem("Edate")
     ...
     var uyy=idd[4]
      ...
      if(!/[0-9]+-[0-9]+-[0-9]{4}/.test(idd))
      {
          ...
          document.getElementById("upedt").innerHTML="Invalid date format(format:dd-mm-yyyy)";
      }
      ...
      {
        document.getElementById("upedt").hidden=false;
        document.getElementById("upedt").innerHTML="Invalid month";
  ...
  const ButtonVali=()=>
  {
     ...
     if(/[A-Za-z]{2}[A-Za-z0-9]/.test(updatevalue.Pname) && updatevalue.Pname.length<50 && updatevalue.Pquality.length<=4 && /[0-9]{3}[%]{1}/.test(updatevalue.Pquality) && /[0-9]/.test(updatevalue.Pcount) && updatevalue.Pcount <=200000 && /[0-9]/.test(updatevalue.Price) && updatevalue.Price <= 10000000 && sessionStorage.getItem("Mdate")==1 && sessionStorage.getItem("Edate")==1)
     {
...
<div className="tr-heading" style={{display:"flex"}}><h6 style={{marginLeft:"1%",fontFamily:"Time New Roman"}}>ADDITEM</h6> <button className="close" style={{marginLeft:"89%"}} onClick={updatemyretForm}><RefreshIcon/></button><button className="close" style={{marginLeft:"0.5%"}} onClick={remove}><CloseIcon/></button></div>
