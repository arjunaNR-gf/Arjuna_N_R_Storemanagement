...
useEffect(() => {
    firebaseDb.child('DEALERLOGIN').on('value', snapshot =>{
                ...
                for(var j=0;j<Iid.length;j++)
                {
                        ...
                        if(Iid[i]==Values.Iid && pass[j]==Values.pass )
                        {
                            ...
                            if(count>0)
                            {
                                setdisplay(true);
                                document.getElementById("ppass").hidden=true;
                                document.getElementById("ppass").innerHTML="password not mactching";
                            }
    ...
    return(
        ...
        <div div className={display ? "menu-inactive":"menu"}>
            ...
            <div className="h4">SignIn</div>
                ...
                <div style={{display:"flex",marginLeft:"5%"}}> <div className="icons-menu"><PersonIcon/></div><input type="text" name="Iid" value={Values.Iid} className="form-control" placeholder="ENTER DEALER ID"  onKeyUpCapture={checkemail} onChange={inputhandle} reqired></input></div>
                <p id="pemail" className="pp1" hidden></p>
                <br></br>
            
                <div style={{display:"flex",marginLeft:"5%"}}> <div className="icons-menu"><LockIcon/></div>  <input type="text" name="pass" value={Values.pass} className="form-control" placeholder="ENTER PASSWORD"  onKeyUpCapture={checkpass} onChange={inputhandle} required></input></div>
